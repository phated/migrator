module.exports.migrate = function (app_root, app_config, options) {
  ...
  if (app_config.postgresql) {
    ...
    // create and connect db
    db = mysql.createConnection(app_config.mysql);
    ...
    function migrate(cb) {
      ...
      db.query('SELECT version FROM schema_migrations', function(err, resp) {
        ...
        if (err) {
          ...
          for (var rl = resp.length; rl > 0; rl--) {
            executed.push(resp[rl - 1].version);
          }
        }
        ...
        var run_migrations = (function(torun) {
          ...
          if (current) {
            ...
            if (executed.indexOf(current.id) < 0) {
              ...
              if (current.sql) {
                current.sql = current.sql.replace(/[\r\n]/g, '').trim();
                current.sql = current.sql.indexOf(';', current.sql.length - 1) !== -1 ? current.sql : current.sql + ';';
